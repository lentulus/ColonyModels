...
| -- | ---- | ---------- | ------ | ---------- | ----- | ------ |
| R-001 | RK4 numerically unstable in some valid `(r, β, c, s₀)` region | Low | High | Analytic-logistic anchor ([AutomatedTests](Phase1AutomatedTests.md) 0.2.1) and Turchin cycle anchor (0.2.2). PBT property P-I-3 ([Phase1PBT.md](Phase1PBT.md)) sweeps the parameter space. | Claude (tests) / project lead (review) | Open |
...
| R-003 | Snapshot-cache reuse drifts from from-scratch replay (cache poisoning) | Med | High | Determinism test ([AutomatedTests](Phase1AutomatedTests.md) 2.1.1, sub-case `determinism`), PBT property P-R-1, UI-observable check ([TestCases](Phase1TestCases.md) 5.3.3). | Claude (tests) | Open |
