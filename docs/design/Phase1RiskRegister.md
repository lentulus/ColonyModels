...
| R-014 | Slice 0 anchor `turchin.cycle.test.ts` bound "first N-peak in [80, 220] yr" is 7 yr too tight: with §17 at Turchin verbatim the actual N-peak is at t ≈ 227 yr. Surfaced at Slice 1.3.4b. Anchor still red at import (Slice 2 dependency on `replayTo`), so the assertion never executes today — but it WILL fail at Slice 2.2.3 when `replayTo` lands. | High (certain to fire) | Low (one-line bound widening) | **Mitigated 2026-05-26 at Slice 2.1.0**: anchor's `BLANK_RUN` synced to §17 verbatim (`N0=0.5, s0=10`) and the peak-window assertion widened to `[180, 280]` yr per Phase1MathDerivations §5. Anchor remains red at import (expected — `./replay` doesn't exist yet); closes at Slice 2.2.3 when the anchor turns green for the right reason. | Claude (Slice 2.1.0; closes at 2.2.3) | Mitigated |
...
| R-016 | Slice 0 anchor `turchin.cycle.test.ts` post-peak assertions (`nextTrough` exists; `peak → trough` interval ≥ 100 yr) were written under pre-1.3.4b params (`s0=1, N=0.2`) that produced oscillatory behaviour. With §17 verbatim (`s0=10, N0=0.5`), the deterministic model is a **single excursion** that settles asymptotically to `(N=k₀=1, S=0)` — no trough, no recurring cycles (Phase1Design §17, Phase1MathDerivations §3.3, Turchin p.123). R-014's Slice 2.1.0 amendment widened the peak window but did not audit these post-peak assertions. Surfaced 2026-05-26 during Slice 2.2.1 implementation: with `replay.ts` written, the anchor flipped from import-red to assertion-red on "no local minimum after the first peak". | High (already fired during 2.2.1) | Low (replace trough check with single-excursion + settling assertions) | **Mitigated 2026-05-26 at Slice 2.2.0**: trough-existence + period assertions replaced with (a) `peakCount === 1`, (b) `\|N(500yr) - 1\| < 0.05` (5% settling to k₀), (c) final-100yr non-increasing (no late oscillation). Peak-window `[180, 280]` and `0 < N_final ≤ 1.5` sanity bounds preserved. Closes at Slice 2.2.3 when the anchor turns green for the right reason. | Claude (Slice 2.2.0; closes at 2.2.3) | Mitigated |
